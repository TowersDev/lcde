...
import { View, Text, Image, Dimensions, Animated, TouchableOpacity, TextInput, ScrollView, Button } from "react-native";
...
export default function Home (props) {
  ...
  const searchBarsInLocation = () => {
    const url  = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?'
        const locationa = `location=${regionChange.latitude},${regionChange.longitude}`;
        const radius = '&radius=2000';
        const type = '&keyword=bar';
        const key = '&key=AIzaSyBWfgqqPQVNzth2HY5cVApgGuIpFGEwFVo';
        const restaurantSearchUrl = url + locationa + radius + type + key;
  ...
  return(
    <View style={styles.container}>
      ...
      >
        {locations && locations.map((market, index) => {
          const scaleStyle = {
            transform: [
              {
          ...
          return (
            <MapView.Marker key={index} coordinate={{latitude: market.geometry.location.lat, longitude: market.geometry.location.lng}} onPress={(e) => onMarkerPress(e)}>
      ...
      {/* <Icon type="material-community" size={13} name="map-marker" iconStyle={styles.iconPrueba} underlayColor="transparent" /> */}
      <Animated.ScrollView
          ref={_scrollView}
          horizontal
          scrollEventThrottle={1}
